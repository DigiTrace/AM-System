...
class AddObjectType extends AbstractType
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        $builder->add('barcode_id', TextType::class, ['label' => 'desc.oid',
            'invalid_message' => 'teshfhgfgfghfghft',
            'attr' => ['autofocus' => true,
                'placeholder' => 'DTXX00000', ],
            'constraints' => new Regex(['pattern' => '/DT(AS|HW|AK|HD)\\d{5}$/i',
                'message' => 'barcode.validation.failed']),
        ])
               ->add('name', TextType::class, ['label' => 'desc.name', 'required' => true, 'constraints' => new NotBlank()])
               ->add('verwendung', TextareaType::class, ['label' => 'desc.ousage',
                   'required' => false, ])
               ->add('notiz', TextareaType::class, ['label' => 'desc.notice',
                   'required' => false, ])
               ...
               ->add('dueDate', DateTimeType::class, ['label' => 'when.object.arrived',
                   'required' => true,
                   'widget' => 'single_text',
                   // 'format' => 'dd.MM.yyyy HH:mm',
                   'data' => new \DateTime(),
               ...
               ->add('bauart', TextType::class, ['label' => 'desc.type',
                   'required' => false, ])
               ->add('formfaktor', TextType::class, ['label' => 'desc.formfactor',
                   'required' => false, ])
               ->add('groessealt', IntegerType::class, ['label' => 'desc.bsize',
                   'required' => false,
               ])
               ...
               ->add('hersteller', TextType::class, ['label' => 'desc.producer',
                   'required' => false, ])
               ->add('modell', TextType::class, ['label' => 'desc.modell',
                   'required' => false, ])
               ->add('sn', TextType::class, ['required' => false,
                   'label' => 'desc.sn'])
               ->add('pn', TextType::class, ['required' => false,
                   'label' => 'desc.pn', ])
               ->add('anschluss', TextType::class, ['label' => 'desc.connection', 'required' => false])
               ->add('save', SubmitType::class, ['label' => 'add.object'])
               ->add('saveandnew', SubmitType::class, ['label' => 'add.object.and.add.new.object'])
               ->add('saveandaddsimilar', SubmitType::class, ['label' => 'add.object.and.add.similar.object'])
               ->add('searchbox', TextType::class, ['required' => false])
               ->add('enablecase', CheckboxType::class, ['required' => false, 'label' => 'desc.enable.relate.to.case'])
        ...
        $formModifierCases = function (FormInterface $form, $cases = null) {
            $form->add('case', ChoiceType::class, [
                'label' => 'case',
                'choices' => $cases,
                'required' => false,
                'placeholder' => false,
...
//done
        ...
        $builder->get('barcode_id')->addEventListener(
            ...
            function (\Symfony\Component\Form\FormEvent $event) use ($formModifierKategorie) {
                ...
                // $kategorien = helper::$kategorienToId;
                $kategorien = [];
                if (false !== stripos($barcode, 'DTAK')) {
                    $kategorien = ['category.record' => helper::$kategorienToId['category.record']];
                }
                if (false !== stripos($barcode, 'DTAS')) {
                    $kategorien = ['category.exhibit' => helper::$kategorienToId['category.exhibit'],
                        'category.exhibit.hdd' => helper::$kategorienToId['category.exhibit.hdd']];
                }
                if (false !== stripos($barcode, 'DTHW')) {
                    $kategorien = ['category.equipment' => helper::$kategorienToId['category.equipment'],
                        'category.container' => helper::$kategorienToId['category.container']];
                }
                if (false !== stripos($barcode, 'DTHD')) {
                    $kategorien = ['category.hdd' => helper::$kategorienToId['category.hdd']];
                }
